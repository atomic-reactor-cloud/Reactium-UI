 ...
 * @since 2.0.0
 */
const DESC = 'Create documentation files.';
...
const HELP = () =>
    console.log(`
Example:
  $ arcli docs -c Alert -d toolkit/components/AlertMolecule
`);
 ...
 * @since 2.0.18
 */
const FLAGS = ['component', 'destination', 'overwrite'];
 ...
 * @since 2.0.0
 */
const SCHEMA = ({ props, prompt }) => {
    const defaultDirectory = path.normalize('toolkit/');
                ...
                ask: () => {
                    try {
                        ...
                        const files = [
                            ...
                            fs.existsSync(
                                path.normalize(path.join(dest, file)),
                            ),
                        );
...
const ACTION = ({ opt, props }) => {
    ...
    const schema = SCHEMA({ props, prompt });
...
const COMMAND = ({ program, props }) =>
    program
        ...
        .option('-c, --component [component]', 'UI Component to document.')
